...
// GenerateTestUsers Helper function to generate test users
...
func CleanupTestData(ctx context.Context, db *mongo.Database) {
	collection := db.Collection(TestUsers)
	_, _ = collection.DeleteMany(ctx, bson.M{})
}

func GenerateComplexTestUsers(count int) []TestUser {
	users := make([]TestUser, count)
	now := time.Now()

	for i := 0; i < count; i++ {
		// Create users with different creation times to test date-based aggregations
		createdTime := now.Add(-time.Duration(i*24) * time.Hour) // Each user created 24 hours apart

		users[i] = TestUser{
			ID:      primitive.NewObjectID(),
			Email:   fmt.Sprintf("complex_test%d@example.com", i),
			Name:    fmt.Sprintf("Complex Test User %d", i),
			Status:  i%3 == 0, // Every third user is active
			Created: createdTime,
			Updated: createdTime,
		}
	}
